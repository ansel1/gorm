...
func (s postgres) HasIndex(ctx context.Context, tableName string, indexName string) bool {
	var count int
	s.db.QueryRowContext(ctx, "SELECT count(*) FROM pg_indexes WHERE tablename = $1 AND indexname = $2", tableName, indexName).Scan(&count)
	return count > 0
}
...
func (s postgres) HasTable(ctx context.Context, tableName string) bool {
	var count int
	s.db.QueryRowContext(ctx, "SELECT count(*) FROM INFORMATION_SCHEMA.tables WHERE table_name = $1 AND table_type = 'BASE TABLE'", tableName).Scan(&count)
	return count > 0
}

func (s postgres) HasColumn(ctx context.Context, tableName string, columnName string) bool {
	var count int
	s.db.QueryRowContext(ctx, "SELECT count(*) FROM INFORMATION_SCHEMA.columns WHERE table_name = $1 AND column_name = $2", tableName, columnName).Scan(&count)
	return count > 0
}
